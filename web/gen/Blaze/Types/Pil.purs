...
import Data.BinaryAnalysis (Address, ByteOffset, Bytes)
...
--------------------------------------------------------------------------------
_CallAddr :: forall a. Prism' (CallDest a) Address
_CallAddr = prism' CallAddr f
  where
    f (CallAddr a) = Just $ a
    f _ = Nothing

_CallFunc :: forall a. Prism' (CallDest a) Function
_CallFunc = prism' CallFunc f
  where
    f (CallFunc a) = Just $ a
    f _ = Nothing
...
data Statement a
  ...
  | Call (CallOp a)
  | DefPhi (DefPhiOp a)
  | BranchCond (BranchCondOp a)
  | Ret (RetOp a)
  | Exit
  | TailCall (TailCallOp a)
...
_BranchCond :: forall a. Prism' (Statement a) (BranchCondOp a)
_BranchCond = prism' BranchCond f
  where
    f (BranchCond a) = Just $ a
    f _ = Nothing

_Ret :: forall a. Prism' (Statement a) (RetOp a)
_Ret = prism' Ret f
  where
    f (Ret a) = Just $ a
    f _ = Nothing

_Exit :: forall a. Prism' (Statement a) Unit
_Exit = prism' (\_ -> Exit) f
  where
    f Exit = Just unit
    f _ = Nothing

_TailCall :: forall a. Prism' (Statement a) (TailCallOp a)
_TailCall = prism' TailCall f
  where
    f (TailCall a) = Just $ a
    f _ = Nothing
--------------------------------------------------------------------------------
